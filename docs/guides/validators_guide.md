...
- 날짜: 접두어 제거와 ongoing deadline 정규화
- detail_url: 유효성 검증
- 경력: "신입/인턴" -> "신입"
- 위치: 여러 지역 -> 첫 번째만
...
**검증 항목:**
- 설립연도: 1900~현재 범위 체크
- 사원수: 숫자를 읽어 비현실적 값만 판별하고, 정상 범위에서는 원문 유지
...
현재 구현은 날짜 형식 전체를 엄격히 검증하기보다, `등록:` / `마감:` 접두어 제거와 ongoing 상태 정규화에 초점을 둔다.
